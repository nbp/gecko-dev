...
AltDataOutputStreamChild::WriteFrom(nsIInputStream *aFromStream, uint32_t aCount, uint32_t *_retval)
{
  ...
  nsresult rv = aFromStream->Read(buffer.BeginWriting(), aCount, _retval);
  if (NS_FAILED(rv)) {
    return rv;
  }
  buffer.SetLength(*_retval);
  Unused << WriteDataInChunks(buffer);
  return NS_OK;
}
...
AltDataOutputStreamChild::WriteSegments(nsReadSegmentFun aReader, void *aClosure, uint32_t aCount, uint32_t *_retval)
{
  ...
  *_retval = 0;
  ...
  while (left > 0) {
    uint32_t read = 0;
    buffer.SetCapacity(left);
    rv = aReader(this, aClosure, buffer.BeginWriting(), *_retval, left, &read);
